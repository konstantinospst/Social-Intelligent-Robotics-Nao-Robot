...
from diart import OnlineSpeakerDiarization, PipelineConfig #SpeakerDiarization, SpeakerDiarizationConfig
...
client = OpenAI(
  api_key='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',
)

# Function to send a prompt to the GPT model
def ask_gpt(prompt):
    response = client.chat.completions.create(
        model="gpt-3.5-turbo-1106", # You can choose different models
        messages=[
            {"role": "system", "content": "You are a nao robot who has to be able to hold a human conversation. We need the robot to have a certain threshold where it decides not to answer the question. This could be because the sentence/question does not make sense. If the robot decides to not answer the question it should return a standardized response where it lets the chat know it is not understanding."},
...
def concat(chunks, collar=0.05): #0.38
    """
...
def colorize_transcription(transcription):
    ...
    response = ask_gpt(sentence[1]).content
    hwnd_active = win32gui.GetForegroundWindow()
    win32api.SendMessage(hwnd_active, WM_APPCOMMAND, None, APPCOMMAND_MICROPHONE_VOLUME_MUTE)
   #nao.tts.request(NaoqiTextToSpeechRequest(response), block=False)
    print(response)
    time.sleep(round(1*(len(response.split())/3)) + 0.25)
    print("\n start speaking")
    win32api.SendMessage(hwnd_active, WM_APPCOMMAND, None, APPCOMMAND_MICROPHONE_VOLUME_MUTE)

    return "\n".join(result)

def return_speaker(transcription):
    for speaker, text in transcription:
        if speaker == -1:
            pass
        else:
...
def return_transcription(transcription):
    result = []
    for speaker, text in transcription:
        result.append(text)
    return "\n".join(result)

def return_speaker_and_transcription(transcription):
    transcript = []
    speakers = []
    # for speaker, text in transcription:
    #     if speaker == -1:
    ...
    win32api.SendMessage(hwnd_active, WM_APPCOMMAND, None, APPCOMMAND_MICROPHONE_VOLUME_MUTE)
    #nao.tts.request(NaoqiTextToSpeechRequest(response), block=False)
    print("Speaker", speakers, "said: ", transcript)
...
asr = WhisperTranscriber(model="tiny",device="cuda")
...
source.stream.pipe(
    ...
    ops.map(lambda x: return_speaker_and_transcription(x)[1])
).subscribe(
